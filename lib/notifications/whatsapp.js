...
export function getNotifyRecipients() {
  ...
  // Primary recipient
  const primary = process.env.WHATSAPP_NOTIFY_NUMBER || '918179738737';
  if (primary) {
  ...
  return recipients.length > 0 ? recipients : ['918179738737'];
}
...
export async function sendWhatsAppMessage({ to, text, template }) {
  ...
  if (!enabled || !accessToken || !phoneNumberId) {
    ...
    console.log(`[WhatsApp Mock] Notification simulated for ${recipient.slice(0, 4)}***${recipient.slice(-3)} (Mock ID: ${mockId})`);
    return {
      success: true,
      messageId: mockId,
