...
export function getNotifyRecipients() {
  ...
  // Primary recipient
  const primary = process.env.WHATSAPP_NOTIFY_NUMBER || '917780245307';
  if (primary) {
  ...
  return recipients.length > 0 ? recipients : ['917780245307'];
}
...
export async function sendAppointmentWhatsAppNotification(appointmentData) {
  ...
  // If approved Meta template is configured
      ...
      components: [
        {
          type: 'body',
          parameters: [
            { type: 'text', text: appointmentData.referenceId },
            { type: 'text', text: appointmentData.name },
            { type: 'text', text: appointmentData.phone },
            { type: 'text', text: patientEmail },
...
export async function sendCallbackWhatsAppNotification(callbackData) {
  const recipients = getNotifyRecipients();
  const templateName = process.env.WHATSAPP_CALLBACK_TEMPLATE;
      ...
      components: [
        {
          type: 'body',
          parameters: [
            { type: 'text', text: callbackData.referenceId },
            { type: 'text', text: callbackData.name },
            { type: 'text', text: callbackData.phone },
            { type: 'text', text: patientEmail },
            { type: 'text', text: reasonText },
            { type: 'text', text: callbackTime }
          ]
        }
      ]
    };
